(function($) {
	...
	$.fn.bootstrapTreeTable = function(options, param) {
		...
		target.load = function(parms){
			//alert("parms:"+parms);
            //alert("url:"+options.url+"    位置为："+options.url.indexOf("systemCode"));
			...
			$.ajax({
				type : options.type,
				url : parms?parms.url:options.url,					//第一次使用初始化的url，之后使用传参进来的url
				...
				success : function(data, textStatus, jqXHR) {
					...
					if(options.hasOwnProperty("ondblclick")){
                        target.find("tbody").find("tr").dblclick(function(){
							var context = {id:$(this).children().last().prev().text(),name:$(this).children().last().text()};
                            options.ondblclick(context);
                        });
					}
